function  [E,H,ms_i,z1,z2,lenz,dz,w, Tsteps] = faraday_simulation(e_r,sigma0,ms0,alpha, field_amplitude ...
    ...
    for n=1:1:2    %Duplication of for loop was done to record two steps for boundry condition
        E(:,4,n)=E(:,4,n)+ amplitude_vec(n)*[cos(w*n*dt); cos(w*n*dt+fie); 0]; %inject the input signal
    
        H(1,2:lenz,n+1)=H(1,2:end,n)+0.5*(E(2,2:end,n)-E(2,1:lenz-1,n));
        H(2,2:lenz,n+1)=H(2,2:lenz,n)-0.5*(E(1,2:lenz,n)-E(1,1:lenz-1,n));
    
        ms_i(:,:,n+1) = runge_kutte_LLG_1(ms_i(:,:,n),H(:,z1+1:z2,n)+H_exte,dt,alpha); % solving LLG step
        if close_system
