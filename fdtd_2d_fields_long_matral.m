...
% the matial defniton
z1=ceil(0.5*lenz); %location of material start
z2=ceil(lenz); %material end
...
Pulse= amplified(W,dt,Pulse,0.5,1);
...
% time_plot_2D_fields(E/max(max(max(E))),dz,z(z1),z(z2),1, 'gaussian 2D');
time_plot_2_fields(E(:,:,1), E(:,:,2),dz,2*epsr-5,5,"E_x","E_y", "reflction in the end_2");
