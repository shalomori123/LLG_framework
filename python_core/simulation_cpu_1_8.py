...
def pulse_gen(power, fea, pulse_width, dt ,lamda = 800e-9 , teta = 0, start_sefty = 4 ):
    """
    input
    :param max_E: max value of the fileds [A/m]
    :param fea: phase defernt betwin E_x and E_y
    :param pulse_width: the whith of the gausian doun to e^-1 of power the max power [sec].
    ...
    peak_location = start_sefty*pulse_width
    # size of fild to given power
    # norm = np.pi**0.5 * pulse_width/2 * np.exp(-pulse_width**2  * w**2) *(2*np.exp(pulse_width**2  * w**2)+ np.cos(2*fea) + 1)
    norm = np.pi**0.5 * pulse_width/2 * (2 + (np.cos(2*fea) + 1)* np.exp(-pulse_width**2  * w**2))
    max_ampltod = (1/norm)**0.5 * (power/heta)**0.5
    turn_m= np.array([np.cos(teta) , -np.sin(teta), np.sin(teta) ,np.cos(teta) ])
    turn_m = turn_m.reshape((2,2))
    pulse = max_ampltod * turn_m @ np.array([1 ,np.exp(-fea*1j)]).reshape(2)
    def out_pulse(n):
        """
        input-
        ...
        """
        r = pulse  * np.exp(- (n*dt - peak_location)**2/( 2*pulse_width**2))* np.exp(1j*w*n*dt)
        return r
    return out_pulse 


def LLG_step(M: np.array, H: np.array, dt, alpha):
    """"""
...
def simulation(z1: int, z2: int, e_r : float, sigma: float, alpha: float, m0: np.array,
               max_E: float, fea : float, pulse_width: float,
               save_location, save_time, close_system: bool,
               dt, simulation_time, lamda = 800e-9 , teta = 0):
    """
    inputs-
    ...
    :param lamda:
    :param teta:
    :param time_cut: How much time steps will be in one time cycle.
    ...
    # system defntion 
    dz = 2*c *dt
    lenz= z2 + int(lamda/dz)
 

    # matrial proprty
    material_vec = np.zeros(lenz)
    material_vec[z1:z2] = 1
    eps_r= 1 + (e_r-1)* material_vec[:-1] 
    eaf = dt/2/eps* (sigma* material_vec[:-1]) /eps_r
    const_for_les_E= ((1-eaf)/(1+eaf))
    const_for_H= (0.5/eps_r/(1+eaf))
    

    
    # plase for run the symolation
    pulse = pulse_gen(max_E, fea, pulse_width, dt, lamda, teta)
    ...
    ms_i_current= np.zeros((z2 -z1, 3))
    # intial the beging magntiztion
    for i in range(3):
        ms_i_current[:,i]= m0[i]/(z2-z1)


    # defing rutenting array:
    # for space indxes
    E_space_return = np.zeros((simulation_time, len(save_location), 3)).astype(complex)
    H_space_return = np.zeros((simulation_time, len(save_location), 3)).astype(complex)
    ms_save_indxes = [i - z1 for i in save_location if (i >=z1 and i< z2)] # find the relvenr indx in for msi
    msi_space_return = np.zeros((simulation_time, len(ms_save_indxes),3)).astype(complex)

    # for time indxes
    E_time_return = np.zeros((len(save_time), lenz, 3)).astype(complex)
    H_time_return = np.zeros((len(save_time), lenz, 3)).astype(complex)
    msi_time_return = np.zeros((len(save_time), z2-z1 , 3)).astype(complex)
    index_to_save_in =0
    
    # propegate

    for n in range(0, simulation_time):
        ...
        ms_i_next = LLG_step(ms_i_current, np.real(H_current[z1:z2,:]), dt, alpha)
        if close_system: # intagrtion line for close sistem B = H + M
            H_next[ z1:z2, :] += ms_i_current - ms_i_next
        # E eqution
        ...
        if n in save_time:
            H_time_return[index_to_save_in, :,:] = H_current
            E_time_return[index_to_save_in, :,:] = E_current
        ...
        for save_ind, space_ind  in enumerate(save_location):
            H_space_return[n, save_ind, :] = H_current[space_ind, :]
            E_space_return[n, save_ind, :] = E_current[space_ind, :]
        for save_ind, space_ind  in enumerate(ms_save_indxes):    
            msi_space_return[n, save_ind, :] = ms_i_current[space_ind, :]
    time_r = H_time_return, E_time_return, msi_time_return
    spece_r = H_space_return, E_space_return, msi_space_return       
    return  time_r , spece_r
...
if __name__ == '__main__':
    ...
    lenz= z2 + time_cut//2
    simulation_time = int(4*z1 + 5*(e_r**0.5)*(z2- z1)+time_cut)
    
    save_location = [4, z1, z2-1, z2+2]
    save_time  = [2*pulse_width//dt, pulse_width//dt +2*z1 ]



    time_res, specr_res = simulation(z1, z2, e_r , sigma, alpha, ms0,
               max_E, fea , pulse_width,
